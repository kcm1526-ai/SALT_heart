...
def main(args: Namespace) -> None:
    ...
    for i, input_path in enumerate(inputs, start=1):
        ...
        keep_values = [104,105,106,107,108] #these are hearts
        pred_np = pred["pred"].numpy()[0]
        mask_heart = np.isin(pred_np, keep_values)
        pred_filtered = np.where(mask_heart, pred_np, 0)
        pred_expanded = np.expand_dims(pred_filtered, axis=0)
        pred_meta = MetaTensor(torch.tensor(pred_expanded), meta=pred["pred"].meta)

        saver = SaveImage(output_dir=args.output_dir, output_postfix="heart")
        saver(pred_meta)
